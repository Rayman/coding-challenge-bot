...
from robots.rayman.search import Move, find_best_moves, direction_to_move, calculate_score_grid
...
def test_calculate_score_grid():
    ...
    assert scores[0, 0] == 0.9090909090909091, 'white'
    assert scores[0, 1] == 0, "enemy that can't be taken"
    assert scores[0, 2] == 1, 'enemy that can be overwritten, highest score'
    assert scores[0, 3] == 0.09090909090909091, 'enemy that converts to white'
...
def test_out_of_bounds():
    grid = np.array([[4]])
    position = (0, 0)
    scores = [s for m, s in find_best_moves(grid, position, 0, 4)]
...
def test_out_of_bounds2():
    grid = np.array([[4, 4]])
    position = (0, 0)
    moves_with_scores = {direction_to_move(m): s for m, s in find_best_moves(grid, position, 0, 4)}
    assert len(moves_with_scores) == 1
...
def test_avoid_own_color():
    ...
    position = (0, 0)
    moves_with_scores = {direction_to_move(m): s for m, s in find_best_moves(grid, position, 0, 4)}
    assert moves_with_scores[Move.UP] > 0 and len(moves_with_scores) == 1
...
def test_prefer_to_take_enemies():
    ...
    position = (0, 0)
    moves_with_scores = {direction_to_move(m): s for m, s in find_best_moves(grid, position, 0, 4)}
    assert moves_with_scores[Move.RIGHT] > 0 and len(moves_with_scores) == 1
...
def test_should_not_get_stuck():
    ...
    moves_with_scores = {direction_to_move(m): s for m, s in find_best_moves(grid, position, 1, 4)}
    assert moves_with_scores[Move.UP] > 0 and len(moves_with_scores) == 1
...
def test_should_not_evaluate_going_back():
    grid = np.array([[4, 0, 0],
                     ...
                     [0, 0, 4]])
    position = np.array([0, 0])
    moves_with_scores = {direction_to_move(m): s for m, s in find_best_moves(grid, position, 2, 4)}
    assert moves_with_scores[Move.UP] > 0 and len(moves_with_scores) == 1
...
def test_can_evaluate_starting_position():
    ...
    grid = np.array([[0, 0, 0],
                     [0, 4, 4]])
    position = np.array([0, 0])
    moves_with_scores = {direction_to_move(m): s for m, s in find_best_moves(grid, position, 3, 4)}
    assert moves_with_scores[Move.UP] > 0 and len(moves_with_scores) == 1
